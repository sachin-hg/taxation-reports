    ...
    try {
      ...
      sortedTxnDates.forEach((txnDate) => {
        ...
        if (validEvents.length > 0) {
          ...
          // might want to consider moment here, and same for parsing dates from transaction statements
          const eventDateStr = momentT(nearestEvent.date).tz('America/New_York').subtract(1, 'days').format('YYYY-MM-DD');

          result[ticker][txnDate.format('YYYY-MM-DD')] = {
          ...
          result[ticker][txnDate.format('YYYY-MM-DD')] = null;
        }
      });
    } catch (error) {
